...
<Steps>
  ...
  <Step title="Banco destino verifica que la cuenta esté activa y pueda recibir pagos">
    "Si, por ejemplo, la información proporcionada en la transferencia para realizar el crédito presenta errores en el tipo de cuenta, el banco debe rechazar la transferencia mediante el endpoint `/reject`, incluyendo un código como `828 - `**Target payment method error.**
      ...
      <Tab title="Error Request">
        ...
        curl -X POST \
            ...
            -d '{
                ...
                "error": {
                "code": 828,
                "message": "Target payment method error"
                }
  ...
  <Step title="Banco Destino Registra el `signer` en Transfiya">
    ...
    <Tabs>
      ...
      <Tab title="Request Alias Signer">
        <Info>
          Se requiere consentimiento del usuario para crear signers con alias.
        </Info>
        ```json
        curl -X POST \
           -H "x-api-key: <API_KEY>" \
