---
title: 'Sobre errores de DICE'
description: 'Guia sobre manejo de errores en DICE'
icon: 'triangle-exclamation'
---

## Resumen

Transfiya usa un sistema de manejo de errores basado en estándar de errores HTTP.

Los sistemas externos como DICE utilizan códigos de error específicos, sin embargo, Transfiya cuenta con un mecanismo que mapea automáticamente estos códigos a su estructura de errores HTTP.
...
<Info>
Para asegurar compatilidad con todos los participantes, validaciones de llaves se van a informarar a travez de warnings para llaves que no tienen consentimento de usuario.
</Info>

Las advertencias siguen el estándar HTTP y se envían en los encabezados de error. Ejemplo de una advertencia:

``` X-Minka-Warning: 118 *labels.type* is required. This will become an error after September 2025 ```

Llaves que tienen consentimento de usuario se van a enviar a DICE
...
La mayoría son errores de configuración, por lo que no deberían ser visibles para los participantes.

El error más común que podría observarse es `U122 - InboundBankNotActive`, el cual ocurre cuando los participantes están fuera de servicio o han sido marcados como suspendidos por cualquier otra razón.
...
<Info>
    Referencia para el mapeo se uso el
    Documento Mecanismo Operativo para la Liquidación – MOL
    Versión 2.1.1
    2025-03-07
</Info>

<Accordion title="Mapeo Errores MOL">
